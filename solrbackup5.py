...
def clusterstate(solr_url):
    return json.loads(getjson(solr_url + '/admin/collections?action=CLUSTERSTATUS&wt=json'))["cluster"]["collections"]
...
def indexversion(solr_url, core):
    response = json.loads(getjson(solr_url + '/%s/replication?command=indexversion&wt=json' % core))
    return {'generation': response['generation'], 'indexversion': response['indexversion']}

def filelist(solr_url, core, version):
    t = getjson(solr_url + '/%s/replication?command=filelist&wt=json&%s' % (core, urlencode(version)))
    return json.loads(t)["filelist"]
